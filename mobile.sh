#!/bin/bash
# ab-tracker — mobile responsive pass
# Run from inside ~/ab-tracker: bash mobile.sh

set -e
cd ~/ab-tracker

echo "→ Updating Sidebar with mobile hamburger..."
...
echo "→ Updating dashboard layout for mobile (push content down for top bar)..."
...
echo "→ Updating BoardClient with mobile tab bar + full-screen detail..."
...
export default function BoardClient({ initialWorkOrders, clients, services, team }: {
  ...
  function openNewWo() {
    setNewWo({
      title: '', stage: 'not-started', priority: 'medium',
  ...
  function handleDragStart(e: React.DragEvent, woId: string) {
    setDraggedId(woId); e.dataTransfer.effectAllowed = 'move'
  }
...
echo ""
echo "✅ Mobile responsive done!"
echo ""
echo "Next: build + push"
echo "  cd ~/ab-tracker && npm run build && git add -A && git commit -m 'Mobile: responsive layout + hamburger + stage tabs' && git push"
echo ""
